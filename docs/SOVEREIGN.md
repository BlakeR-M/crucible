...
| Adversarial verifier | `gpt-5` (2nd pass) | **gpt-oss-20b**, a *different family* from the planner by design | **Nemotron 3 Ultra GenRM** or a cross-family pairing |
| Serving | OpenAI API | **llama.cpp `llama-server`** (dev) → **vLLM** (real) | **vLLM** or **SGLang**; TensorRT-LLM only when frozen |

**The honest headline:** on a single 16 GB card you can reach roughly `gpt-5-mini` class capability for worker tasks and *most* of it for planning. You cannot reach `gpt-5` class. The gap varies by task: it is small for *judging* (1–2 points) and large for *generating a correct hard fix* (13+ points, see §7). A sovereign deployment that matches `gpt-5` on this workload needs 2× 96 GB-class GPUs minimum and lands around **USD 30–45k of hardware**, or **USD 370–450k** for an 8-GPU HGX node if the agency wants headroom and concurrency.
...
1. **The Qwen line went through two more generations.** Qwen3.5 shipped February 2026, Qwen3.6 in April 2026. Both are Apache 2.0. The architecture moved to a hybrid **Gated DeltaNet + sparse MoE** design that materially improves the "small active parameter count, large total parameter count" tradeoff, which is what a 16 GB card needs.
...
| Qwen3.5-4B / 2B | 4B / 2B | ~5.5 / ~3.5 GB | Apache 2.0 | For classification, routing and extraction. See the §7 warning on small-model quantisation |
...
1. **Qwen3.6-35B-A3B (planner) ↔ gpt-oss-20b (verifier)**: different labs, different training data and a different architecture family. Both fit the 16 GB budget if run sequentially, or the verifier can live on CPU/second card.
...
Source: [Qwen3.5 GGUF benchmarks (Unsloth)](https://unsloth.ai/docs/models/qwen3.5/gguf-benchmarks)
...
| **Structured output (JSON schema)** | Yes via `response_format`, though *enforcement is the weak spot* | Yes (GBNF grammars) | Yes, more complete than Ollama | Yes since v0.8.5; **xgrammar** default backend | Yes | Yes |
...
| **Model swap at runtime** | Yes | Yes | Yes | One model per process | One per process | **No**, the engine is compiled per GPU + dtype |
...
- **FlashAttention 3 does not work on Blackwell yet**, so set `VLLM_FLASH_ATTN_VERSION=2`.
...
- There is a known bad CUDA release: **avoid CUDA 13.2, which produces gibberish output on Qwen3.6; use 13.1 or 13.3.**
...
3. **Use SGLang instead of vLLM if your agent prompts share long system prefixes**, which orchestration systems almost always do. A fixed 4,000-token system prompt across 20 agents is the case RadixAttention was built for.
4. **Reach for TensorRT-LLM only when the model, the GPU, and the quantisation are all frozen**: a production accreditation boundary rather than a development machine. The 10–30 minute per-config compile and the non-portable engines are the cost of the last 30–50%.

**Wire up the structured-output backend.** Failure rates for JSON conformance, measured:
...
> Source: [modelfit RTX 5080 profile](https://modelfit.io/gpu/rtx-5080/), 13 Aug 2026

**KV cache budget:** +~0.5 GB at 16K context, +~2.0 GB at 64K. Both fit alongside the recommended models. **Plan a 128K-context agent on this card only if you accept a large throughput loss**, since a 27B at IQ3_XXS drops from 45 t/s at 19K to **9.6 t/s at 128K**.

**The single most important number in this section:** a model that fits entirely in VRAM is **3–11× faster** than one that offloads. gpt-oss-20b at 139.93 tok/s versus gpt-oss-120b with heavy CPU offload at 12.64 tok/s on the same box is the canonical illustration. Design the local tier around *fitting* rather than around *the biggest model you can technically load*.
...
**Critical framing for the documentation, because it is widely misreported:** `--n-cpu-moe` makes a *non-fitting* model *possible*, and on a model that already fits it costs speed. The famous "5× speedup" is only ever measured against the alternative of generic layer offload. Once the model fits, every increment of `--n-cpu-moe` costs throughput monotonically.
...
| Qwen3.5-27B (dense) | UD-IQ3_XXS | 128K | n/a | 9.6 |
...
These cannot all be resident simultaneously on one 16 GB card. Options: (a) sequential swap via Ollama/LM Studio's model manager, ~5–15 s per swap; (b) planner on GPU, worker on CPU (a 9B at Q4 runs 8–15 tok/s on a modern 16-core CPU, acceptable for background workers); (c) accept the swap cost and design the orchestrator's phases around it. **Option (c) is usually right**, because planning, working and verifying are naturally sequential phases in most agent loops.
...
**For the defence audience, this is the argument to lead with:** for a government agency the case for sovereign deployment rests on the compliance boundary rather than on the economics. Frame the cost honestly and let the boundary carry the decision. An agency spending $370k on an HGX H200 node is not saving money against `gpt-5` API calls at typical volumes; it is buying the ability to process data that legally cannot leave the boundary at all.
...
The standard air-gapped stack is straightforward and worth stating because it reassures: **Linux + NVIDIA drivers/CUDA, vLLM or NVIDIA NIM for serving, Prometheus/Grafana for monitoring**, all installable inside an air-gapped enclave with no egress.
...
**The migration is a base-URL change plus a grammar backend.** Every stack in §3 exposes `/v1/chat/completions`. What breaks in practice sits above the transport:
...
|---|---|---|---|---|
| **gpt-5** | **74.9%** | 88.4% (89.4 thinking) | 94.6% | not listed here |
| **gpt-5-mini** | **48.0%** | 68.7 | not published | $0.125/M in, $1.00/M out, 400K ctx |
...
### 7.3 Where local falls short for *this* workload
...
Top of BFCL v3 as of 29 June 2026: **GLM 4.5 at 76.7%**, Claude Opus 4.7 at 76.6%, Gemini 3.1 Flash Lite Preview at 76.5%. Open weights are at the top of this leaderboard, though *nobody* is above 77%. Multi-turn tool use remains the weakest link in agent systems regardless of provenance, and a sovereign deployment does not make it worse. It does mean the orchestrator needs retry and repair logic either way.

**(d) The frontier is still ahead, and by a measurable amount.**

NIST's CAISI evaluation of DeepSeek V4 puts open-weight capability **~8 months behind the frontier**. That is the honest number to quote: it comes from an allied government evaluator and it is specific, which makes it far more credible than "open models have caught up."
...
- Verifier: `gpt-oss-20b` MXFP4, its native precision. Run it as shipped rather than requantising.
...
- `Nemotron-3-Ultra-GenRM` as the verification tier, with the review rubric expressed as user-defined principles, so the rubric itself stays inspectable, version-controlled and open to audit.
...
- [Qwen3.6: How to Run Locally (Unsloth)](https://unsloth.ai/docs/models/qwen3.6)
- [Qwen3.5 GGUF Benchmarks (Unsloth)](https://unsloth.ai/docs/models/qwen3.5/gguf-benchmarks)
- [Dynamic 2.0 GGUFs (Unsloth)](https://unsloth.ai/docs/basics/unsloth-dynamic-2.0-ggufs)
...
- [`--n-cpu-moe`: Run a Big MoE Model on a Small GPU (Aliteq, Jul/Aug 2026)](https://aliteq.com/run-big-moe-model-small-gpu-n-cpu-moe-guide)
...
- [NVIDIA DGX Spark Review: $4,699 (IntuitionLabs)](https://intuitionlabs.ai/articles/nvidia-dgx-spark-review)
...
- [vLLM vs SGLang 2026: RadixAttention vs PagedAttention (Spheron)](https://www.spheron.network/blog/vllm-vs-sglang-2026/)
...
- [LLM Inference On-Premise vs GPU Cloud: 2026 Cost and Break-Even (Spheron)](https://www.spheron.network/blog/llm-inference-on-premise-vs-cloud/)
...
- [Data Residency for AI Workloads: Australian Compliance and Sovereign Deployment (Brightlume)](https://brightlume.ai/blog/data-residency-ai-workloads-australian-compliance-sovereign-deployment)
...
- [Adversarial Code Review: Why the Maker Shouldn't Grade the Checker (Augment Code)](https://www.augmentcode.com/guides/adversarial-code-review)
- [Autonomous Code Review: Multi-Agent Approaches to PR Analysis (Zylos Research, 22 Apr 2026)](https://zylos.ai/research/2026-04-22-autonomous-code-review-multi-agent-pr-analysis/)